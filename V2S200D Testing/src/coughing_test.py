#Joseph de la Viesca
#Coughing test for V2S200D Voice vibration sensor
...
#Recording setup
duration = int(len(coughing_audiodata)/Fs) #5 second recording
...
write(filename = os.path.join(recs_dir, f"Coughing audio played from speaker (bone conduction mic recording).wav"), rate = Fs, data = rec_bone)
write(filename = os.path.join(recs_dir, f"Coughing audio played from speaker (air mic recording).wav"), rate = Fs, data = rec_air)
#Generate spectrograms
f_air_plot, t_air, Sxx_air_plot = spectrogram(rec_air, fs=Fs, nperseg=2048)
f_bone_plot, t_bone, Sxx_bone_plot = spectrogram(rec_bone, fs=Fs, nperseg=2048)
...
#Waveforms
axes[0, 0].plot(time_axis, rec_air, color='blue')
axes[0, 0].set_title("Air mic - Waveform")
axes[0, 0].set_xlabel("Time [s]")
axes[0, 0].set_ylabel("Amplitude")
axes[0, 0].grid(True)
axes[0, 1].plot(time_axis, rec_bone, color='green')
...
pcm1 = axes[1, 0].pcolormesh(t_air, f_air_plot, 10 * np.log10(Sxx_air_plot + 1e-10), shading='gouraud', cmap='plasma')
...
pcm2 = axes[1, 1].pcolormesh(t_bone, f_bone_plot, 10 * np.log10(Sxx_bone_plot + 1e-10), shading='gouraud', cmap='plasma')
...
plt.tight_layout()
plt.savefig(os.path.join(plots_dir, f"Coughing audio waveform + spectrogram.png"))